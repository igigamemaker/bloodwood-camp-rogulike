...
const state={running:false,paused:false,time:0,wave:1,waveTime:0,lastWave:1,spawnTimer:0,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:0,kills:0,paidRun:false,bossSpawnedWave:0,bossCount:0,mapIndex:0,portalSpawned:false,finalBossSpawned:false,bossesDefeatedThisMap:0,totalXpGained:0,heartSpawnTimer:15,currentProfile:null,waveWaitingForBoss:false,waveSpawnRemaining:0,waveAdvanceLock:0};
...
const HERO_BASE_STATS={mage:{hp:100,dmg:12.5,mana:100,manaRegen:2,speed:5.05},guard:{hp:125,dmg:15,mana:50,manaRegen:1,speed:4.65},archer:{hp:85,dmg:10,mana:85,manaRegen:1,speed:5.45}};
...
const shopItems=[{name:'Sharper Starter Bow',desc:'+1 permanent starting damage. Cost starts at 100 coins and increases by 100 each level.',base:100,key:'dmg',apply:()=>meta.dmg++},{name:'Wheel of Fortune',desc:'Max 3 spins. Each future run starts with one random active or passive ability.',base:500,costs:[500,500,500],key:'startSkill',max:3,apply:()=>meta.startSkill++}];
...
const bossHpByOrdinal=[1000,1250,1500,1750,2500,2000,2250,2500,2750,3500,3000,3250,3500,3750,4500,4000,4250,4500,4750,5500,5000,5250,5500,5750,10000];
...
function abilityPreview(a){
  ...
  if(n==='Sharper Runes'){const bonuses=[1.10,1.15,1.20,1.25,1.30], l=player.upgrades.sharperRunes||0, cur=player.damage, next=l<5?player.damage/(l>0?bonuses[l-1]:1)*bonuses[l]:player.damage;return line(`${cur.toFixed(1)} basic dmg`,`${next.toFixed(1)} basic dmg`)}
  ...
  if(n==='Twin Arrows')return line(`${player.bullets} projectile(s), ${player.damage.toFixed(1)} dmg each`,`${Math.min(6,player.bullets+1)} projectiles, ${player.damage.toFixed(1)} dmg each`);
...
function spawnEnemy(){if(state.portal)return;const av=getAvailableEnemyTypes(),t=chooseEnemyType(av),a=Math.random()*Math.PI*2,d=15+Math.random()*10;
  ...
  const spdProgress=(1+state.wave*.014+state.time*.00035+state.mapIndex*.014)*0.72;
...
function bossSpecialAttack(e,dt){
  ...
  e.specialCd=(e.finalBoss?3.35:(special==='summon'?8.2:6.5))+Math.random()*(e.finalBoss?1.0:(special==='summon'?2.2:2.0));
...
function bossSlotOnCurrentMap(finalFight=false){
  ...
  return Math.max(1,Math.min(5,(state.bossesDefeatedThisMap||0)+1));
}
...
function spawnBoss(finalFight=false){if(state.portal)return;sfx('boss');state.bossCount++;
  ...
  const e={tex:profile.tex,name:profile.name,x:finalFight?4:player.x+Math.cos(a)*d,z:finalFight?0:player.z+Math.sin(a)*d,
    ...
    special:skills[0]||'charge',specials:skills,specialCd:finalFight?1.75:3.6,attackDelayUntil:finalFight?state.time+3:0};
...
function clearAdminMapRuntime(){
  ...
  state.portal=null;state.portalSpawned=false;state.bossSpawnedWave=0;state.finalBossSpawned=false;state.bossesDefeatedThisMap=0;state.spawnTimer=0;state.heartSpawnTimer=15;
}
...
function drop(kind,x,z,amount=1){if(kind==='heart'&&heartDropCount()>=3)return null;const size=kind==='levelCrystal'?1.05:(kind==='heart'?.55:.48);const d={kind,x,z,amount,mesh:sprite(textures[kind],size)};entityGroup.add(d.mesh);state.drops.push(d);return d}
...
function weightedChoices(pool,count){const maxed=a=>{const u=player.upgrades||{};return (a.name==='Dancing Dagger'&&player.spells.dagger>=5)||(a.name==='Spirit Wisps'&&player.spells.wisps>=5)||(a.name==='Poison Spores'&&player.spells.spores>=5)||(a.name==='Rune Mines'&&player.spells.mines>=5)||(a.name==='Force Field'&&player.spells.force>=5)||(a.name==='Fireball Tome'&&player.spells.fireball>=5)||(a.name==='Frost Nova Charm'&&player.spells.frost>=5)||(a.name==='Storm Acorn'&&player.spells.lightning>=5)||(a.name==='Fleet Boots'&&player.speedLevel>=5)||(a.name==='Faster Draw'&&player.attackSpeedLevel>=5)||(a.name==='Sharper Runes'&&(u.sharperRunes||0)>=5)||(a.name==='Blue Tea'&&(u.blueTea||0)>=5)||(a.name==='Moon Magnet'&&(u.moonMagnet||0)>=5)||(a.name==='Warm Soup'&&(u.warmSoup||0)>=5)||(a.name==='Meteor Rain'&&player.spells.meteor>=1)||(a.name==='Twin Arrows'&&player.class==='guard')||(a.name==='Twin Arrows'&&(u.twinArrows||0)>=5)||(a.name==='Piercing Candle'&&player.class!=='archer')||(a.name==='Piercing Candle'&&(u.piercingCandle||0)>=5)||(a.name==='Axe Reach'&&player.class!=='guard')||(a.name==='Axe Reach'&&(player.guardRangeLevel||0)>=5)};const result=[],copy=pool.filter(a=>!maxed(a));while(result.length<count&&copy.length){const total=copy.reduce((s,a)=>s+(a.weight||1),0);let r=Math.random()*total,idx=0;for(;idx<copy.length;idx++){r-=copy[idx].weight||1;if(r<=0)break}result.push(copy.splice(Math.min(idx,copy.length-1),1)[0])}return result}
...
function makeSaveSnapshot(){
  ...
  const keepPlayer=['class','maxHp','maxMana','speed','level','xp','nextXp','money','damage','fireRate','attackSpeedLevel','bullets','pierce','bulletSpeed','magnet','manaRegen','crit','spread','speedLevel','guardRange','guardRangeLevel','upgrades','spells','meteorUsedMap'];
...
function restoreGameSnapshot(snap,opts={}){
  ...
  state.bossSpawnedWave=0; state.waveAdvanceLock=.35; state.heartSpawnTimer=15;
...
function reset(cls){document.body.classList.remove('title-flow');if(cls)player.class=cls;[...state.enemies,...state.bullets,...(state.enemyShots||[]),...state.drops,...state.particles,...state.spells,...state.orbiters].forEach(removeEntityVisuals);Object.assign(state,{running:true,paused:false,time:0,wave:1,waveTime:0,lastWave:1,spawnTimer:0,enemies:[],bullets:[],drops:[],particles:[],spells:[],enemyShots:[],orbiters:[],portal:null,lastShot:-9,kills:0,paidRun:false,bossSpawnedWave:0,bossCount:0,mapIndex:0,portalSpawned:false,finalBossSpawned:false,bossesDefeatedThisMap:0,totalXpGained:0,heartSpawnTimer:15,uiTimer:0,waveSpawnRemaining:0,waveAdvanceLock:0});const base=HERO_BASE_STATS[player.class]||HERO_BASE_STATS.mage;const startHp=base.hp,startMana=base.mana;Object.assign(player,{x:-4,z:4,hp:startHp,maxHp:startHp,mana:startMana,maxMana:startMana,speed:base.speed,level:1,xp:0,nextXp:24,money:0,damage:base.dmg,fireRate:.48,attackSpeedLevel:0,bullets:1,pierce:0,bulletSpeed:13,magnet:2.2,manaRegen:base.manaRegen,crit:.05,spread:.13,invuln:0,speedLevel:0,guardRange:1.45,guardRangeLevel:0,upgrades:{},spells:{fireball:0,frost:0,lightning:0,dagger:0,wisps:0,spores:0,mines:0,force:0,meteor:0},cooldowns:{fireball:0,frost:0,lightning:0,meteor:0,wisp:0,spore:0,mine:0,force:0},meteorUsedMap:-1});applyClassStats(player.class);makeDungeon();state.waveSpawnRemaining=waveMobTarget();grantStarterSkills();notice('Wave 1 started — clear all enemies to start the next wave. Boss every 2 waves in asset order.');ui.welcomeModal.classList.add('hidden');ui.instructionModal.classList.add('hidden');ui.startModal.classList.add('hidden');ui.gameOverModal.classList.add('hidden');ui.victoryModal.classList.add('hidden');ui.levelModal.classList.add('hidden')}
function grantStarterSkills(){const picks=abilityDefs.map(a=>a.id);for(let i=0;i<Math.min(3,meta.startSkill||0);i++){const p=picks[Math.floor(Math.random()*picks.length)];if(p==='meteor')player.spells.meteor=1;else if(p==='dagger'){player.spells.dagger=Math.min(5,(player.spells.dagger||0)+1);syncDaggers()}else player.spells[p]=Math.min(5,(player.spells[p]||0)+1);const def=abilityDefs.find(a=>a.id===p);toast('Wheel of Fortune: '+(def?def.name:p));notice('Starting skill: '+(def?def.name:p))}}
function renderShop(){ui.shop.innerHTML='';if(ui.finalStats){ui.finalStats.innerHTML=`Level ${player.level}, wave ${state.wave}, ${state.kills} monsters defeated, $${player.money} collected.<br>Merchant coins: $${meta.gold}`}for(const item of shopItems){const level=meta[item.key]||0,max=item.max??99,cost=item.costs?item.costs[Math.min(level,item.costs.length-1)]:item.base+level*item.base;const btn=document.createElement('button');btn.disabled=level>=max;btn.innerHTML=`<b>${item.name}</b><small>${item.desc}</small><small>Level ${level}${item.max?'/'+item.max:''} · Cost: $${level>=max?'MAX':cost}</small>`;btn.onclick=()=>{if(level>=max)return;if(meta.gold>=cost){meta.gold-=cost;item.apply();saveMerchantMeta();renderShop();updateUi();toast('Bought '+item.name)}else toast('Not enough coins')};ui.shop.appendChild(btn)}}
function startClass(cls){clearControls();reset(cls||'mage')}
if(ui.newGameBtn)ui.newGameBtn.addEventListener('click',()=>{audioStart();sfx('ui');openNewGameProfile()});if(ui.loadGameBtn)ui.loadGameBtn.addEventListener('click',()=>{audioStart();sfx('ui');openLoadGame()});if(ui.profileCreateBtn)ui.profileCreateBtn.addEventListener('click',()=>{audioStart();sfx('ui');createProfileAndChooseHero()});if(ui.profileBackBtn)ui.profileBackBtn.addEventListener('click',()=>{ui.profileModal.classList.add('hidden');ui.welcomeModal.classList.remove('hidden')});if(ui.loadBackBtn)ui.loadBackBtn.addEventListener('click',()=>{ui.loadModal.classList.add('hidden');ui.welcomeModal.classList.remove('hidden')});if(ui.welcomeNextBtn)ui.welcomeNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.welcomeModal.classList.add('hidden');if(ui.storyModal)ui.storyModal.classList.remove('hidden');else ui.instructionModal.classList.remove('hidden')});if(ui.storyNextBtn)ui.storyNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.storyModal.classList.add('hidden');ui.instructionModal.classList.remove('hidden')});ui.instructionNextBtn.addEventListener('click',()=>{audioStart();sfx('ui');ui.instructionModal.classList.add('hidden');ui.startModal.classList.remove('hidden')});ui.mageBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('mage')});ui.guardBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('guard')});ui.archerBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass('archer')});ui.restartBtn.addEventListener('click',()=>{audioStart();sfx('ui');continueFromDeathCheckpoint()});ui.victoryRestartBtn.addEventListener('click',()=>{audioStart();sfx('ui');startClass(player.class||'mage')});
...
function maybeStartBossWave(){const isFinalMap=currentMap().decor==='colosseum';const activeBoss=state.enemies.some(e=>e.boss);if(state.wave%2===0&&state.bossSpawnedWave!==state.wave&&!state.portalSpawned&&!activeBoss){state.bossSpawnedWave=state.wave;state.waveWaitingForBoss=true;const slot=(state.bossesDefeatedThisMap||0)+1;if(isFinalMap){if(slot<=4)safeSpawnBoss(false);else if(slot>=5&&!state.finalBossSpawned){state.finalBossSpawned=true;safeSpawnBoss(true)}}else if(slot<=BOSSES_PER_NORMAL_MAP)safeSpawnBoss(false)}}
...
function gameOver(){sfx('death');clearControls();state.running=false;if(!state.paidRun){meta.gold+=player.money;state.paidRun=true;saveMerchantMeta()}ui.finalStats.innerHTML=`Level ${player.level}, map ${Number(state.mapIndex||0)+1}, wave ${state.wave}, ${state.kills} monsters defeated, $${player.money} collected.<br>Merchant coins: $${meta.gold}<br><small>Continue restarts from the beginning of this map.</small>`;renderShop();if(ui.restartBtn)ui.restartBtn.textContent='Continue From This Map';ui.gameOverModal.classList.remove('hidden')}
...
function renderSideStats(){
  ...
  if(u.twinArrows)push(abilityIconData.twin,'Twin Arrows','Lv '+u.twinArrows+'/5',abilityRarity['Twin Arrows']);
