...
arrowProjectile:imageTexture('assets/fx/player_arrow.png'),
axeProjectile:imageTexture('assets/fx/guard_axe.png'),
...
coin:imageTexture('assets/fx/drop_gold.png'),
xp:imageTexture('assets/fx/drop_xp.png'),
levelCrystal:imageTexture('assets/fx/drop_level_up.png'),
heart:imageTexture('assets/fx/drop_heart.png'),
...
abilityFireball:imageTexture('assets/fx/player_fireball.png'),
...
demonFireball:imageTexture('assets/fx/boss_fireball.png'),
mageBossAttack:imageTexture('assets/fx/mage_orb.png'),
meteorImpact:imageTexture('assets/fx/meteor_impact.png'),
...
function castMeteorRain(){const l=player.spells.meteor;if(!l)return toast('Find Meteor Rain first.');if(player.meteorUsedMap===state.mapIndex)return toast('Meteor Rain can be used once per map.');if(player.mana<activeManaCosts.meteor)return toast('Not enough mana for Meteor Rain.');player.mana-=activeManaCosts.meteor;player.meteorUsedMap=state.mapIndex;player.cooldowns.meteor=9999;sfx('meteor');let killed=0;const targets=state.enemies.filter(e=>!e.boss);for(const e of targets){for(let i=0;i<2;i++){const m=sprite(textures.meteorImpact,.82);m.position.set(e.x+(Math.random()-.5)*1.2,.24,e.z+(Math.random()-.5)*1.2);entityGroup.add(m);state.spells.push({mesh:m,life:.55,scale:.25})}e.hp=0;killed++;burst(e.x,e.z,0xff733f,18)}cleanupDead();toast('Meteor Rain!');notice('Meteor Rain cleared '+killed+' normal mobs. It will refresh on the next map.')}
