...
const HERO_BASE_STATS={mage:{hp:100,dmg:12.5,mana:100,manaRegen:2,speed:4.65},guard:{hp:125,dmg:15,mana:50,manaRegen:1,speed:4.25},archer:{hp:85,dmg:10,mana:85,manaRegen:1,speed:5.0}};
...
function applyMerchantDamageUpgrade(){const base=HERO_BASE_STATS[player.class]||HERO_BASE_STATS.mage;const starter=base.dmg+(meta.dmg||0);const sharper=player.upgrades&&player.upgrades.sharperRunes? [1.10,1.15,1.20,1.25,1.30][Math.min(4,player.upgrades.sharperRunes-1)] : 1;player.damage=starter*sharper;updateUi&&updateUi();}
...
function spawnEnemy(){if(state.portal)return;const av=getAvailableEnemyTypes(),t=chooseEnemyType(av),a=Math.random()*Math.PI*2,d=15+Math.random()*10;
  ...
  const spdProgress=(1+state.wave*.014+state.time*.00035+state.mapIndex*.014)*0.82;
...
function spawnBoss(finalFight=false){if(state.portal)return;sfx('boss');state.bossCount++;
  ...
  const e={tex:profile.tex,name:profile.name,x:finalFight?4:player.x+Math.cos(a)*d,z:finalFight?0:player.z+Math.sin(a)*d,
    ...
    spd:profile.spd*speedProgress*1.12,
...
function spawnMapMerchant(){
  ...
  const base=state.portal;
  let x=base.x+2.2,z=base.z;
  if(!canStandAt(x,z,.65)){x=base.x-2.2;z=base.z}
  const mesh=sprite(textures.merchantGoblin,2.7);
  mesh.scale.set(3.9,2.7,1);
  mesh.position.set(x,1.05,z);
  const label=textSprite('Merchant - enter to spin','#ffe7aa');
  label.position.set(x,2.65,z);
  const glow=circle(1.25,0xffcf7b,.18);
  glow.position.set(x,.08,z);
  entityGroup.add(glow);entityGroup.add(mesh);entityGroup.add(label);
  state.merchant={x,z,mesh,label,glow,timeLeft:10,opened:false};
  state.mapMerchantSpins=0;
  notice('Merchant appeared near the portal for 10 seconds. Walk into him to spin once.');
}
...
function resetRunProgressAfterDeath(){
  ...
  if(had.twin&&cls!=='guard'){player.upgrades.twinArrows=1;player.bullets=2}
  if(had.sharper){player.upgrades.sharperRunes=1;player.damage*=1.10}
